        ...
        return [
          ...
          ...(prevState = [
            <Msg
              MsgAction="input"
              User="BotServer"
              MsgValue={"Hey there 👋, How can I help you?"}
              key={prevState.length}
            />,
            <Msg
              MsgAction="items"
              MsgItems={["Admission", "Placement", "Syllabus", "College Fee"]}
        ...
        if (intent.up_suggestions.length > 0) {
          setMsgList((prevState, index) => {
            return [
              ...prevState,
              ...(prevState = [
                <Msg MsgAction="preLoader" key={prevState.length} />,
              ]),
            ]
          })
          setTimeout(() => {
            setMsgList((prevState, index) => {
              return [
                ...
                ...(prevState = [
                  <Msg
                    MsgAction="items"
                    MsgItems={intent.up_suggestions}
                    key={prevState.length}
                    selectItem={selectItem}
                    setSelectItem={setSelectItem}
                  />,
  ...
  useEffect(async () => {
    ...
    if (dataTemp.length > 0) {
      ...
      setMsgList((prevState, index) => {
        return [
          ...prevState,
          ...(prevState = [<Msg MsgAction="preLoader" key={msgList.length} />]),
        ]
      })
  ...
  return (
    <>
      <div
        className={
          props.action
            ? "absolute bottom-[60px] sm:bottom-0 right-[110px] sm:right-0 shadow-[0_35px_60px_-5px_rgba(0,0,0,0.7)] w-[370px] sm:w-screen sm:h-screen  h-[600px] bg-secondary rounded-2xl"
            : "hidden"
        }
      >
        <div className="flex flex-col bg-secondary rounded-2xl sm:rounded-none h-full w-full justify-between p-2">
          <div className="h-[15%] flex relative bg-white rounded-xl shadow-lg ">
            <div className="w-1/4">
              <img
                src="images/Chatbot/chatButton.png"
                className={`${BasicStyles.hugeShadow} animate-bounce2 absolute h-22 w-24 -top-2`}
                alt=""
              />
            </div>
            <div className="w-3/4 flex flex-col p-2 pl-5 relative">
              <div className="font-bold text-3xl text-[#3e395a]">Mista</div>
              ...
              <div className="flex flex-row justify-end items-center text-slate-400 text-xs gap-2 pr-8">
                <p className="tracking-wider">Your KITCOEK Assistant</p>
          ...
          <div
            id="chatBox"
            className=" overflow-x-clip h-[90%] bg-transparent rounded-xl flex flex-col  overflow-y-auto p-3 space-y-4"
          >
            <div className="text-xs my-0 mx-auto py-1 px-4  bg-[#eee] text-[#999] rounded-[32px] w-fit">
              Today
            </div>
            {msgList}
          </div>
          ...
          <div className="h-[15%] bg-transparent rounded-xl mt-2 ">
            <div className="flex flex-col">
              <div className="flex flex-row space-x-2">
                <input
                  onKeyPress={(e) => e.key === "Enter" && submit.click()}
                  type="text"
                  value={inputField}
                  onChange={(e) => setInputField(e.target.value)}
                  className=" bg-white h-10 rounded-3xl w-5/6 shadow-lg flex items-center justify-center p-4 focus:outline-none"
                      ...
                      setMsgList(
                        msgList.concat(
                          <Msg
                            ...
                            key={msgList.length}
                          />,
                          <Msg MsgAction="preLoader" key={msgList.length + 1} />
                        )
                      )
                      setInputField("")
                ...
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className=" rotate-90 h-5 w-5 text-[#3e395a]"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
              ...
              <div className="flex mt-3 justify-center">
                <p className="flex flex-row text-xs text-slate-500">
                  Powered by{" "}
                  <a href="www.hashinclude.co.in" className="flex flex-row">
                    {" "}
                    <img
