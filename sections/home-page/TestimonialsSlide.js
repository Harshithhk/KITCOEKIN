...
const TestimonialSlide = (props) => {
  const testimonials = [
    ...
    },
    {
      ...
      name: 'Bob Johnson',
      company: '123 Co.',
      quote: 'Suspendisse eu libero quam. Nunc accumsan dui eget augue rhoncus egestas.',
    },
    {
      id: 4,
      name: 'Sarah Lee',
      company: '456 Inc.',
      quote: 'Phasellus accumsan sodales neque, vel tempor diam lacinia quis.',
    },
    {
      id: 5,
      name: 'Tom Brown',
      company: '789 Co.',
      quote: 'Proin tempus purus ac tellus commodo, vel bibendum turpis auctor.',
    },
  ];
  ...
  const handleClick = (direction) => {
    if (direction === 'left') {
      setSlideIndex(slideIndex > 0 ? slideIndex - 1 : testimonials.length - 1);
    } else {
      setSlideIndex(slideIndex < testimonials.length - 1 ? slideIndex + 1 : 0);
    }
  };
    ...
    <div className=" px-32 sm:px-1  py-16 mt-20 text-[#24346D]">
      <div className="mx-auto px-4 sm:px-6 2xl:px-8">
        ...
        {testimonials.map((testimonial, index) => (
          <div key={index} className={`testimonial  ${index === slideIndex ? 'active' : 'hidden'}`}>
            <p className="text-base sm:text-base 2xl:text-lg font-medium  sm:flex-col flex items-center justify-center gap-3 ">
              {/* {testimonial.quote} */}
              <span className=''>
            ...
            <div className="mt-8  gap-3 flex justify-center">
              ...
              <div className="ml-4 ">
                ...
                <div className="text-base font-medium text-[#24346D]">{testimonial.position}</div>
              </div>
            </div>
          </div>
        ))}
